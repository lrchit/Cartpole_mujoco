...
class PentaDiagonalFactorization {
  ...
  /// to this method.
  void SolveInPlace(ocs2::vector_t* b);
  ...
  bool SolverSucceeded(const Eigen::LDLT<BlockType>& solver) { return solver.info() == Eigen::Success; }
  ...
  ocs2::vector_t r_;  // Convenient storage for the Solve() stage.
  PentaDiagonalFactorizationStatus status_{PentaDiagonalFactorizationStatus::kFailure};
};

template <int kBlockSize>
PentaDiagonalFactorization<kBlockSize>::PentaDiagonalFactorization(const PentaDiagonalMatrix<double>& M) {
  Factorize(M);
  r_.resize(M.block_size() * (M.block_rows() + 2));
}

template <int kBlockSize>
void PentaDiagonalFactorization<kBlockSize>::Factorize(const PentaDiagonalMatrix<double>& M) {
...
template <int kBlockSize>
void PentaDiagonalFactorization<kBlockSize>::SolveInPlace(ocs2::vector_t* b) {
  assert(b->size() == size());
  ...
  ocs2::vector_t& rm2 = r_;  // rm2[i] stores r_{i-2}.
