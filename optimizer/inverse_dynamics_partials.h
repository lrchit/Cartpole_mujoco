...
struct InverseDynamicsPartials {
  ...
  InverseDynamicsPartials(const int num_steps, const int nv, const int nq) {
    assert(num_steps > 0);
    dtau_dqm.assign(num_steps, ocs2::matrix_s_t<T>::Zero(nv, nq));
    dtau_dqt.assign(num_steps, ocs2::matrix_s_t<T>::Zero(nv, nq));
    dtau_dqp.assign(num_steps, ocs2::matrix_s_t<T>::Zero(nv, nq));
  ...
  //
  std::vector<ocs2::matrix_s_t<T>> dtau_dqm;
  ...
  //
  std::vector<ocs2::matrix_s_t<T>> dtau_dqt;
  ...
  //
  std::vector<ocs2::matrix_s_t<T>> dtau_dqp;
};

template struct InverseDynamicsPartials<double>;
